...
func NewStatusCode(code int32) *Status {
...
func (s *Status) Description() string {
	...
	case StatusInvalidArgument:
		return "The client specified an invalid argument" // Note that this differs from FAILED_PRECONDITION. INVALID_ARGUMENT indicates arguments that are problematic regardless of the state of the system (e.g., a malformed file name)."
	case StatusDeadlineExceeded:
		return "The deadline expired before the operation could complete" // For operations that change the state of the system, this error may be returned even if the operation has completed successfully. For example, a successful response from a server could have been delayed long"
	case StatusNotFound:
		return "Some requested entity was not found" // Note to server developers: if a request is denied for an entire class of users, such as gradual feature rollout or undocumented allowlist, NOT_FOUND may be used. If a request is denied for some users within a class of users, such as user-based access control, PERMISSION_DENIED must be used."
	case StatusAlreadyExists:
		return "The entity that a client attempted to create already exists"
	case StatusPermissionDenied:
		return "The caller does not have permission to execute the specified operation" //. PERMISSION_DENIED must not be used for rejections caused by exhausting some resource (use RESOURCE_EXHAUSTED instead for those errors). PERMISSION_DENIED must not be used if the caller can not be identified (use UNAUTHENTICATED instead for those errors). This error code does not imply the request is valid or the requested entity exists or satisfies other pre-conditions."
	...
	case StatusUnavailable:
		return "The service is currently unavailable" //This is most likely a transient condition, which can be corrected by retrying with a backoff. Note that it is not always safe to retry non-idempotent operations."
	case StatusDataLoss:
